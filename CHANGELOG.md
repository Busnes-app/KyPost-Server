...
- Bind browser PGP and password writes to the snapshot used for preparation. Preserve prepared recovery revisions across tab switches and upload failures; refuse missing revision support. Add a narrow password-change snapshot so forced resets remain possible while preserving old sealed PGP material for recovery.
