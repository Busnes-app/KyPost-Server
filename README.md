...
- Send-as aliases, each verified before use: a code is mailed as the alias to the alias through your own outgoing server, and the user types it back, or the daemon accepts the same message if it loops back DKIM-signed by the alias's domain
...
- `GET|POST /api/mail/send-as`, `POST /api/mail/send-as/{id}/confirm` and `DELETE /api/mail/send-as/{id}` (alias addresses. A new alias is unusable until the user confirms the mailed code, or a DKIM-signed copy from its own domain reaches the inbox. The list never returns the code.)
...
- `GET /api/notifications/native/pull` (every notification is queued here in both modes, so a device that stops hearing from the relay can poll and catch up)
