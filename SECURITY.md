...
1. Go to the [Security Advisories](https://github.com/Busnes-app/KyPost-Server/security/advisories) page
...
- An SSO session also ends when the identity provider says so: `POST /api/auth/oidc/backchannel-logout` accepts a signed `logout+jwt` for this client, records its `jti` on disk so a replay after a restart revokes nothing, and ends the session with that `sid` (or all of the subject's sessions issued before the token, when it carries no `sid`). A logout that lands while the browser is still completing the same login fences that login for five minutes. Paired devices and CardDAV app passwords are not derived from the SSO session and are revoked by directory deactivation, not by logout.
- Directory offboarding removes access, never data. A KySignOn `user.updated{active:false}` or `user.deleted` is a signed, versioned event (`POST /api/sync/webhook`); it deactivates the account and revokes every session, device, CardDAV password and pairing token, and the mailbox, keys and account record stay, so a rehire restores the same account (one with a local password re-authorizes its SSO link from a local sign-in, as after any credential revocation). The last active admin can be neither removed nor demoted by the directory. A stale or replayed event is refused by a revision fence that survives restarts, and a sign-in for a subject the directory disabled, or with a token issued before its access last changed, is refused.
- Deactivation or demotion takes effect on the user's next authenticated request.
- An SSO session is admin only when both the account and the ID token say so: KySignOn's `kypost.admin` app role (or an admin group from Authentik/Keycloak) is recorded on the session as a ceiling, and a session without it acts as a regular user even if the account is an administrator. KySignOn's global admin role is never mapped to KyPost admin.
- A session signed in through KySignOn itself (Authentik and Keycloak sessions keep the password gate) has no password to re-enter, so the actions that ask for one (the Security page, backup) send it back to KySignOn: a challenge bound to the exact request, a fresh sign-in in a popup (`prompt=login`; recovery-code sign-ins do not count), and a one-time grant spent only by that session for that request. PGP identity operations still require the password, which is also the vault credential.
...
- **Vulnerability reports:** [GitHub Security Advisories](https://github.com/Busnes-app/KyPost-Server/security/advisories)
