...
- Directory offboarding removes access, never data. A KySignOn `user.updated{active:false}` or `user.deleted` is a signed, versioned event (`POST /api/sync/webhook`); it deactivates the account and revokes every session, device, CardDAV password and pairing token, and the mailbox, keys and account record stay. A stale or replayed event is refused by a revision fence that survives restarts, and a sign-in for a subject the directory disabled, or with a token issued before its access last changed, is refused.
