...
- A session signed in through KySignOn has no password to re-enter, so the actions that ask for one (the Security page, backup) send it back to KySignOn: a challenge bound to the exact request, a fresh sign-in in a popup (`prompt=login`; recovery-code sign-ins do not count), and a one-time grant spent only by that session for that request. PGP identity operations still require the password, which is also the vault credential.
